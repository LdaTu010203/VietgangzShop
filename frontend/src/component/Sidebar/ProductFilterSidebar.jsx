...
const ProductFilterSidebar = ({
  ...
  return (
    <div className="w-full md:w-64 bg-white shadow-lg rounded-lg p-4 sticky top-16 h-[calc(100vh-64px)] overflow-y-auto">
      <h2 className="text-xl font-bold mb-4 text-gray-800 uppercase tracking-wide">
        Bộ lọc
      </h2>
      ...
      <div className="mb-6">
        ...
        {openSections.category && (
          <div className="mt-2 space-y-2">
            {(expandedCategories ? categories : categories.slice(0, 5))
              .filter((cat) => cat.status)
              .map((cat) => (
                <div
                  key={cat._id} // Sử dụng _id làm key
                  className={`flex items-center space-x-2 p-2 rounded-lg border border-gray-300 hover:bg-gray-100 hover:shadow-md transition-all duration-300 ${
                ...
                className={`w-8 h-8 rounded-full cursor-pointer border border-gray-300 
                              ${
                                filters.color.includes(color)
                                  ? "ring-2 ring-indigo-600 scale-110 shadow-lg"
                                  : ""
                              } 
